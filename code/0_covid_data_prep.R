...
move  %>% filter(!Species %in% pant$Binomial) %>% select(Species) %>% distinct() # elk and desert bighorn sheep is not included 
...
move1d <- move %>%
  ...
  filter(n >= 0.75 * 24) %>%
  select(ID, first_timestamp, one_day_displacement_km) %>%
...
move_reshape <- move %>%
  ...
  # filter(time_diff < 15 & time_diff > 5) %>% # 14,753  --> 11,533. # this step can save more data if we get raw data from marlee
  filter(time_diff < 1.25 & time_diff > 0.75) %>% # 20,440 --> 18,071
  mutate(
    ID,
  ...
  ungroup() %>%
  drop_na(end_long) # 18,071 -> 17,881
